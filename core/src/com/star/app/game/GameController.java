...
public class GameController {
    public static final int SPACE_WIDTH = 5000;
    public static final int SPACE_HEIGHT = 3000;
    ...
    public boolean checkPhysicHit(Ship s, Asteroid a) {
        if (a.getHitArea().overlaps(s.getHitArea())) {
            float dst = a.getPosition().dst(s.getPosition()); // смотрим расстояние между ними
    ...
    public void checkCollisions(float dt) {
        for (int i = 0; i < asteroidController.getActiveList().size(); i++) {
            ...
            if (checkPhysicHit(bot, a)) { // если они сталкнулись
                a.takeDamage(2);
